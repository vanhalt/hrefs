class LinksController < ApplicationController
  ...
  def index
    ...
    @pagy, @links = pagy(Link.search(@query).order(created_at: :desc), limit: 15)
  end

  # GET /links/1 or /links/1.json
  def show
    @pagy_visits, @recent_visits = pagy(@link.visits.recent_first, page_param: :visits_page, limit: 10)
  end
  ...
  private
    ...
    def link_params
      params.expect(link: [ :name, :slug, :destination_url, :active, :expires_at ])
    end
end
